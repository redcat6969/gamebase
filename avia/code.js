...
function focusEditorWindow() {
    var _a, _b;
    const api = figma;
    (_b = (_a = api.activeWindow) === null || _a === void 0 ? void 0 : _a.focus) === null || _b === void 0 ? void 0 : _b.call(_a);
}
async function getAnnotationSide() {
...
function configureLayoutFrame(layoutFrame, side, noteWidth) {
 ...
 */
function configureLinkRow(linkRow, noteW) {
...
function applyLayoutFrameChildrenSizing(link, note, side, linkRow) {
    if (side === "below" || side === "above") {
        const row = linkRow;
 ...
 */
function getAnnotationLinkStart(frame, layoutFrame, note, side) {
...
function getPerpendicularTowardFrameDir(side) {
    switch (side) {
        case "below":
...
function intersectRayWithRectBoundary(sx, sy, dirX, dirY, fx, fy, fw, fh) {
    const eps = 1e-6;
...
function getAnnotationLinkEndpoints(frame, layoutFrame, note, side) {
...
async function setAnnotationLinkGeometry(link, layoutFrame, frame, note, side, linkRow) {
 ...
 */
function positionLayoutFrameRelativeToFrame(frame, layoutFrame, side) {
    const { x: fx, y: fy, width: fw, height: fh } = frame;
...
async function createNote() {
    ...
    if (side === "below" || side === "right") {
        layoutFrame.appendChild(linkRow !== null && linkRow !== void 0 ? linkRow : link);
